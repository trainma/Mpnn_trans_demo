'''
Implementing the LSTM baseline method in Keras and Tensorflow with custom dataset.
2022/05/18 xhz.
...
def get_dataset_multi_step2(Source, scope, train_test_split, window_size, n_out):
    split = int(train_test_split * scope)
    df = Source
    scaler = MinMaxScaler()
    scaler2 = MinMaxScaler()
    open_arr = scaler.fit_transform(df.values)
    # label_list = np.zeros(shape=(scope - window_size - n_out, df.shape[1]))
    label_list = np.zeros(shape=(scope - window_size - n_out, n_out, df.shape[1]))
    for i in range(scope - window_size - n_out):
        label_list[i, :] = open_arr[i + window_size:i + window_size + n_out, :]

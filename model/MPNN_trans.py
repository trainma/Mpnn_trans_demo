...
class MPNN_trans(nn.Module):
    ...
    def forward(self, X, adj_edge_index, adj_edge_attr,enc_self_mask=None, dec_self_mask=None, dec_enc_mask=None):
        ...
        x = x.permute(2, 3, 0, 1).contiguous().view(-1, self.window, self.nfeat)
        # [bz seq_len feature]
        x_enc = x
        seq_y = self.data
        x_dec = torch.zeros(x.shape[0], self.pred_len, 4)

        enc_out = self.enc_embedding(x_enc,x_mark_enc)  # x_enc.shape([128(bz), 96(seq_len), 6(feature_dim)]) x_mark_enc.shape([64, 96, 4]) enc_out [64 96 512]
